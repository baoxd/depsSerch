        ...
        try {
            if (aliaspath) {
                ...
                if (webpackConfig.resolve && webpackConfig.resolve.alias) {
                    resolveAlias = webpackConfig.resolve.alias;
                }
                if (!alias && resolveAlias) {
                    alias = resolveAlias;
                } else if (alias && resolveAlias) {
                    for (let k in resolveAlias) {
                        alias[k] = resolveAlias[k];
                    }
                }
            }
        } catch(e) {
...
function readFileCallback(code, file) {
    ...
    let depsPath = parse.parseStr(code, file, alias);
    if (!depsPath) {
        depsPath = [];
    }
    ...
    depsPath = depsPath.map(val => {
        ...
        try {
            // 文件夹
            // 判断是否是文件夹
            const depSate  = fs.statSync(depPath);
            if (depSate.isDirectory()) {
                // index引入的两种情况
                // 1: import "./A/"
...
function isRequireedByWebPack(dep) {
    ...
    function _isRequired(entry) {
        ...
        if (utils.isObject(entry) && Object.keys(entry).length > 0) {
            const keys = Object.keys(entry);
            for (let i = 0 ; i < keys.length; i++) {
                const key = keys[i];
                const value = entry[key];
                if (utils.isString(value)) {
                    let filepath = _getAbsolutelyPath(value);
                    if (filepath.indexOf(dep) >= 0) {
                        return true;
                    }
                } else if (utils.isArray(value) && value.length) {
                    for (let j = 0 ; j < value.length ; j++) {
                        const v = value[j];
                        if (utils.isString(v)) {
                            let filepath = _getAbsolutelyPath(v);
                            if (filepath.indexOf(dep) >= 0) {
                                return true;
                            }
                        }
...
function isUseAlias(filepath) {
    ...
    if (aliasKeys && aliasKeys.length > 0) {
        aliasKeys.forEach(k => {
            if (filepath.indexOf(k) === 0) {
                isUsed = true;
            }
        });
    }
    return isUsed && filepath.indexOf('.') !== 0 && filepath.indexOf('/') > 0;
}
